...
## Pattern 4: Combining Data from Multiple Nodes

**Usage**: When you need data from different nodes in a single Code node

**IMPORTANT**: Since n8n v2.x, `$node["Name"].json` is **unreliable inside Code nodes**. Use a **Merge node** upstream to combine inputs, then access them via `$input`.

### Recommended Approach: Merge Node + $input

```javascript
// Step 1: Add a Merge node before the Code node
// Connect Node A → Merge (Input 1)
// Connect Node B → Merge (Input 2)
// Connect Merge → Code node

// Step 2: Access merged data in Code node
const allItems = $input.all();

// Items from Input 1 come first, then Input 2
const fromNodeA = allItems.filter((_, i) => i < countFromA);
const fromNodeB = allItems.filter((_, i) => i >= countFromA);

return [{
  json: {
    combined: {
      a: fromNodeA.map(i => i.json),
      b: fromNodeB.map(i => i.json)
    }
  }
}];
```

### Alternative: Use $input with Named Merge Modes

```javascript
// If Merge is set to "Combine by Position" (mode: combineBySql or combine)
// Each item has data from both branches
const items = $input.all();
const combined = items[0].json; // Contains fields from both inputs

return [{json: combined}];
```

### Legacy Pattern (DEPRECATED - may not work in v2.x)

```javascript
// ⚠️ $node is unreliable in Code nodes since n8n v2.x
// Only valid in EXPRESSION FIELDS (outside Code nodes)
// const webhookData = $node["Webhook"].json; // May fail
```
