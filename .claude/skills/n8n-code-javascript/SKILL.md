...
### Pattern 4: $node - Reference Other Nodes (DEPRECATED in Code nodes)

**WARNING**: `$node["Name"].json` is **unreliable inside Code nodes** since n8n v2.x. Use a Merge node to combine data from multiple nodes, then access via `$input`.

```javascript
// ❌ DEPRECATED in Code nodes (n8n v2.x+)
const webhookData = $node["Webhook"].json;

// ✅ CORRECT: Use Merge node upstream, then access via $input
const allItems = $input.all();
const webhookItem = allItems[0].json; // First input (from Merge)
const httpItem = allItems[1].json;    // Second input (from Merge)
```

**Note**: `$node["Name"].json` remains **valid in expression fields** (outside Code nodes). The restriction applies only to JavaScript/Python Code node execution context.

**See**: [DATA_ACCESS.md](DATA_ACCESS.md) for the updated guide
