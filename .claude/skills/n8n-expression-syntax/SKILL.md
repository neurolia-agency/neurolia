...
Access midnight of today (useful for date comparisons):

```javascript
{{$today}}                              // Midnight today (DateTime)
{{$today.toFormat('yyyy-MM-dd')}}       // "2026-02-23"
{{$today.plus({days: 1})}}              // Tomorrow midnight
{{$today.diff($json.createdAt, 'days')}} // Days since creation
```

**Difference with $now**:
- `$now` = current date + time (e.g., 2026-02-23T14:30:00)
- `$today` = today at midnight (e.g., 2026-02-23T00:00:00)
