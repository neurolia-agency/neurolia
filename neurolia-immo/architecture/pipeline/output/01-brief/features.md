...
| F02 | Gestion des proprietes | CRUD proprietes : adresse, code d'acces, WiFi, URLs iCal (Airbnb + Booking), checklists d'entretien par bien, consignes specifiques. Choix du mode de check-in par bien (auto-gestion voyageur ou accueil par le personnel). | Aucune |
...
| F12 | Creation automatique taches entretien | Generation automatique des taches menage a chaque checkout detecte via n8n. Attribution selon planning intelligent (F08). Creation automatique d'une tache `check_in_greeting` si `property.check_in_mode = staff_checkin` lors de chaque nouvelle reservation. | Aucune (backend n8n) |
...
- **Formulaire pre-arrivee** : Doublon avec les plateformes Airbnb/Booking qui collectent deja heure d'arrivee, demandes speciales et nombre de voyageurs
