...
## Flow 2ter : Accueil Voyageur

**Scenario** : Le bien est configure en `check_in_mode = staff_checkin`. A chaque reservation, une tache `check_in_greeting` est creee automatiquement par n8n (WF04). Le personnel accueille le voyageur sur place.

```
[Notification push]
  "Nouvelle tache : Studio Plage
   Accueil voyageur, 16h"
         |
         v
  [Planning du jour]
   ...
  ...
  [Fiche Intervention]
   ...
   |     a l'heure prevue       |
   | [ ] Faire la visite guidee |
   |     du logement            |
   | [ ] Remettre les cles /    |
   |     expliquer le digicode  |
   | [ ] Montrer les            |
...
| # | Ecran | Action Utilisateur | Reponse Systeme | Feature |
|---|-------|-------------------|-----------------|---------|
| 1 | Notification | Recoit notification nouvelle tache d'accueil | Push avec nom du bien, type de tache, heure, badge "Accueil" | F10 |
| 2 | Planning | Voit la tache dans son planning | Tache inseree chronologiquement, badge "Accueil" (couleur distincte des badges "Menage" et "Ponctuel") | F07 |
| 3 | Fiche Intervention | Tap sur la tache | Ouvre Fiche Intervention avec checklist accueil (5 items specifiques) | F07 |
| 4 | Fiche Intervention | Execute la checklist | Meme flow que Flow 2 (Commencer → cocher items → Terminer) | F07 |
| 5 | Validation | Tap "Terminer" | Statut "Termine", notifie le proprietaire, retour planning | F07 |

### Decisions UX

- **Meme parcours, badge different** : Les taches d'accueil suivent exactement le meme flow que les taches menage et ponctuelles. Seul le badge change : "Accueil" au lieu de "Menage depart" / "Check arrivee" / "Ponctuel".
- **Checklist dediee** : La checklist affichee est la checklist d'accueil par defaut (5 items). Si le proprietaire a personnalise la checklist `check_in_greeting` pour ce bien, c'est celle-ci qui est utilisee.
- **Horaire** : L'heure prevue correspond a `reservation.arrival_time` ou 16:00 par defaut.
