...
| 3 | Fiche Bien (zone basse) | Tap "Modifier" | Formulaire pre-rempli (adresse, code, WiFi, URLs iCal, mode d'accueil). Section Accueil : toggle "Le personnel accueille les voyageurs" (active → `check_in_mode = staff_checkin`, desactive par defaut → `self_checkin`). Si active, texte d'aide : "Une tache d'accueil sera creee automatiquement pour chaque reservation." | F02 |
