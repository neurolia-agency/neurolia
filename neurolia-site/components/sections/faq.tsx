...
/**
 * LANDING FAQ - "THE REASSURANCE"
 *
 * Positioned between ContactMini and CtaFinal.
 * 4 questions focused on lifting final objections before conversion.
 * Accordion-style, same visual DNA as the rest of the site.
 */
...
function FaqItem({
  ...
  return (
    <div
      className="group border-b border-[rgba(245,245,245,0.06)] cursor-pointer hover:bg-[rgba(245,245,245,0.02)] transition-colors duration-300 px-2 sm:px-4"
      onClick={onToggle}
      onKeyDown={(e) => {
    ...
    >
      <div className="flex items-center justify-between py-4 sm:py-5 md:py-6">
        <div className="flex items-center gap-3 sm:gap-4">
          <span className="text-sm font-bold text-[#C45C3B] tracking-[-0.02em] shrink-0 w-5 sm:w-6 text-right">
        ...
        <span className="relative w-5 h-5 shrink-0 ml-4">
          <span className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-2.5 sm:w-3 h-px bg-[#C45C3B]" />
          <span
            className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-2.5 sm:w-3 h-px bg-[#C45C3B] transition-transform duration-300 ${isOpen ? "rotate-0" : "rotate-90"}`}
          />
        </span>
      </div>
      <AnimatePresence initial={false}>
        {isOpen && (
          ...
          >
            <p className="pb-6 pl-2 sm:pl-10 text-sm md:text-base text-[#A3A3A3] leading-[1.7] sm:max-w-2xl font-light">
              {answer}
            </p>
          </motion.div>
        )}
...
export default function Faq() {
  ...
  return (
    ...
    >
      ...
      <div
        ref={containerRef}
        className="px-2 sm:px-6 md:px-12 lg:px-20 max-w-[1100px] mx-auto"
      >
        {/* Section header */}
        <motion.div
        ...
        >
          <span className="inline-flex items-center gap-4 mb-3">
            <span className="w-8 h-px bg-[#C45C3B]" />
            <span className="text-[11px] sm:text-[10px] tracking-[0.3em] uppercase text-[#C45C3B] font-medium">
