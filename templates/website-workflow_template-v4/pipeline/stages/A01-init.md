...
- `input/forms/` (réponses de formulaires externes — si disponibles)
...
2. **Détecter** les formulaires dans `input/forms/` — si présents :
   a. Lire le manifest pour comprendre le mapping
   b. Extraire uniquement les données mappées vers A01
   c. Croiser avec brief-client.md (le brief prévaut en cas de contradiction)
   d. Signaler si CSV sans manifest (dans "Éléments Manquants")
3. **Analyser** les assets fournis (logos, cartes, contenus existants) — en extraire les informations exploitables
4. **Identifier** le type de site, les pages nécessaires et le positionnement
5. **Rechercher** le contexte concurrentiel et le marché (si données disponibles)
6. **Rédiger** le brief structuré selon le template ci-dessous
7. **Lister** explicitement les éléments manquants avec leur impact sur le pipeline
...
| Source | Type | Exploité en A01 | Données restantes pour |
|--------|------|-----------------|----------------------|
| [fichier ou "Aucun"] | [brand-platform / strategy-audit / —] | [Oui/Partiel/Non] | [A02-Brand / A03 / —] |
...
- [ ] Sources externes listées (formulaires dans input/forms/)
